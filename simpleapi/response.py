...
__all__ = ['ResponseElement', 'Response', 'BaseResponse']

import string, sys, re, json
from xml.etree import cElementTree as ET

def _validate_element(el):
    if not isinstance(el, ResponseElement):
        raise TypeError("element must be a ResponseElement")

class BaseResponse(dict):
    pass

class ResponseElement(BaseResponse):
    """
    ResponseElement class. This is used for all items to be sent across
    the network and is wrapped in a Response before being sent across
    ...
    def __init__(self, tag, text=None, **kwargs):
        ...
        @usage;
            ResponseElement('example')
            ResponseElement('example', 'some text contents')
            ResponseElement('example', 'some text contents', attr1=1, ...)
        """
        super(ResponseElement, self).__init__(**kwargs)
        self.tag = tag
        self._root = self
        self._children = list()
        if text:
    ...
    def __repr__(self):
        return "<ResponseElement({tag}) at 0x{id:x}>".format(tag=self.tag,
                                                       id=id(self))
    ...
    def __setattr__(self, attr, value):
        ...
        if attr not in ['_tag', 'tag', '_text', 'text', '_parent', 'parent',
            ...
            super(ResponseElement, self).__setitem__(attr, value)
        else:
            super(ResponseElement, self).__setattr__(attr, value)
    ...
    def clear(self):
        ...
        """
        super(ResponseElement, self).clear()
        self._children = []
        self.text = None
    ...
    def parse_json(cls, source):
        """
        Parse the source and turn it into a proper ResponseElement
    ...
    def _set_tag(self, value):
        ...
        self._tag = value
    tag = property(_get_tag, _set_tag, doc="The ResponseElement tag")
    ...
    text = property(_get_text, _set_text, _del_text,
                    doc="The ResponseElement text")

    def _get_children(self):
        return list(self._children)
    children = property(_get_children, None, doc="The ResponseElements children")
    ...
    parent = property(_get_parent, None, _del_parent,
                      doc="The ResponseElements parent")
    ...
    root = property(_get_root, None, _del_root,
                    doc="The ResponseElements root")

class Response(BaseResponse):
    """
    Response wrapper class.  This class represents an entire message
    ...
    def __setattr__(self, attr, value):
        ...
        if attr not in ['_text', 'text', '_root', 'root', '_errors', 'errors']:
            super(Response, self).__setitem__(attr, value)
        else:
            super(Response, self).__setattr__(attr, value)

    def __repr__(self):
        return '<Response at 0x{id:x}>'.format(
    ...
    def _get_root(self):
        return self._root
    root = property(_get_root, None, doc="The root ResponseElement or None")
